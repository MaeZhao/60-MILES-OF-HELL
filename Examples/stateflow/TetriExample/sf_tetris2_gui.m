    ...
    switch varargin{1}
        ...
        case 'init'
            if isempty(tetrisFig) || ~ishandle(tetrisFig) || ~strcmp(get(tetrisFig, 'Tag'), 'StateflowTetrisFigure')
                ...
                waitAxis = axes;
                waitShapeImage = image(zeros(SHAPE_WIDTH,SHAPE_WIDTH), 'Parent', waitAxis);
                statusAxis = axes;
        ...
        case 'draw'
            ...
            if all([ishandle(arenaImage) ishandle(waitShapeImage) ishandle(scoreText)])
        ...
        case 'keypress'
            ch = double(get(tetrisFig, 'CurrentCharacter'));
            if isempty(ch)
                return;
            end
